...
import (
	...
	"github.com/chwjbn/livesrv/protocol/amf"
)
...
func (connClient *ConnClient) readRespMsg() error {
	...
	for {
		...
		case 20, 17:
			...
			glog.InfoF("readRespMsg: vs=%v", vs)
			for k, v := range vs {
				switch v.(type) {
				case string:
...
func (connClient *ConnClient) writeConnectMsg() error {
	...
	glog.InfoF("writeConnectMsg: connClient.transID=%d, event=%v", connClient.transID, event)
...
func (connClient *ConnClient) writeCreateStreamMsg() error {
	...
	glog.InfoF("writeCreateStreamMsg: connClient.transID=%d", connClient.transID)
	...
	for {
		...
		if err == ErrFail {
			glog.InfoF("writeCreateStreamMsg readRespMsg err=%v", err)
			return err
		}
	}
...
func (connClient *ConnClient) writePlayMsg() error {
	...
	glog.InfoF("writePlayMsg: connClient.transID=%d, cmdPlay=%v, connClient.title=%v",
...
func (connClient *ConnClient) Start(url string, method string) error {
	...
	glog.Info("connection:", "local:", conn.LocalAddr(), "remote:", conn.RemoteAddr())

	connClient.conn = NewConn(conn, 4*1024)

	glog.Info("HandshakeClient....")
	if err := connClient.conn.HandshakeClient(); err != nil {
		return err
	}

	glog.Info("writeConnectMsg....")
	if err := connClient.writeConnectMsg(); err != nil {
		return err
	}
	glog.Info("writeCreateStreamMsg....")
	if err := connClient.writeCreateStreamMsg(); err != nil {
		glog.Info("writeCreateStreamMsg error", err)
		return err
	}

	glog.Info("method control:", method, av.PUBLISH, av.PLAY)
