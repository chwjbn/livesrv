...
import (
	...
	"github.com/chwjbn/livesrv/container/ts"
	"github.com/chwjbn/livesrv/parser"
)
...
func NewSource(info av.Info) *Source {
	...
	go func() {
		err := s.SendPacket()
		if err != nil {
			glog.Error("send packet error: ", err)
			s.closed = true
		}
	}()
	return s
...
func (source *Source) DropPacket(pktQue chan *av.Packet, info av.Info) {
	glog.WarnF("[%v] packet queue max!!!", info)
	...
	glog.Info("packet queue len: ", len(pktQue))
}
...
func (source *Source) SendPacket() error {
	defer func() {
		glog.InfoF("[%v] hls sender stop", source.info)
		if r := recover(); r != nil {
			glog.Warn("hls SendPacket panic: ", r)
		}
	}()

	glog.InfoF("[%v] hls sender start", source.info)
	for {
		if source.closed {
...
func (source *Source) Close(err error) {
	glog.Info("hls source closed: ", source.info)
